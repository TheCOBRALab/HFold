	...
	switch (cur_interval->type){
		...
		for (cand_pos_t i=1; i<=j-1; i++)
		{
			...
			switch (best_row)
			{
				...
				case 2:
					...
					if (best_i-1 >= 1)// Hosna, March 26, 2012, was best_i-1 instead of best_i //As is should be because acc is always W[i-1] - Mateo
						insert_node (1, best_i-1, FREE);
					break;
				case 3:
		...
		case M_WMv:
		{
			...
			if(params_->model_details.dangles == 1){
				...
				if(tree.tree[j].pair<0){
					tt = pair[S_[i]][S_[j-1]];
					energy_t tmp = V->get_energy(i,j-1) + E_MLstem(tt,-1,sj,params_) + params_->MLbase;
					if(tmp<min){
						min = tmp;
						best_row = 3;
					}
				...
				if(tree.tree[i].pair<0 && tree.tree[j].pair<0){
					...
					energy_t tmp = V->get_energy(i+1,j-1) + E_MLstem(tt,si,sj,params_) + 2*params_->MLbase;
					if(tmp<min){
						min = tmp;
						best_row = 4;
					}
